...
open class ILBanner: UIView {
    ...
    open var _position: BannerPosition = .top
    ...
    private func addGestureRecognizers() {
        addGestureRecognizer(UITapGestureRecognizer(target: self, action:  #selector(ILBanner.didTap(_:))))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(ILBanner.didSwipe(_:)))
        swipe.direction = .up
    ...
    private func adjustHeightOffset() {
        ...
        if superview === ILBanner.topWindow() && self._position == .top {
    ...
    open func show(_ view: UIView? = ILBanner.topWindow(), duration: TimeInterval? = nil) {
